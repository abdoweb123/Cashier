...
@section('content')
    ...
    <div class="container text-center">
        ...
        <div class="div_main_a">
            ...
            <a href="{{route('best.seller')}}" class="btn" id="reports">المنتجات الأكثر مبيعا</a>
            ...
            <div class="my-container" id="general_view-content" >
                ...
                <div id="tap3-content">
                    <table class="table">
                        ...
                        <tbody>
                        <tr>
                            @if(count($maxSellToday) == true)
                                @foreach($maxSellToday as $item)
                                    <td>{{$item['name']}}</td>
                                @endforeach
                            @else
                                <td>لا يوجد</td>
                            @endif
                            @if(count($maxSellYesterday) == true)
                                @foreach($maxSellYesterday as $item)
                                    <td>{{$item['name']}}</td>
                                @endforeach
                            @else
                                <td>لا يوجد</td>
                            @endif
                            @if(count($maxSellThisMonth) == true)
                                @foreach($maxSellThisMonth as $item)
                                    <td>{{$item['name']}}</td>
                                @endforeach
                            @else
                                <td>لا يوجد</td>
                            @endif
                            @if(count($maxSellLastMonth) == true)
                                @foreach($maxSellLastMonth as $item)
                                    <td>{{$item['name']}}</td>
                                @endforeach
                            @else
                                <td>لا يوجد</td>
                            @endif
                            @if(count($maxSellAll) == true)
                                @foreach($maxSellAll as $item)
                                    <td>{{$item['name']}}</td>
                                @endforeach
                            @else
                                <td>لا يوجد</td>
