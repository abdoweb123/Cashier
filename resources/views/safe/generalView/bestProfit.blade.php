...
@section('content')
    ...
    <div class="container text-center">
        ...
        <div class="div_main_a">
            ...
            <a href="{{route('profits')}}" class="btn inactive" id="collected_data">الأرباح</a>
            <a href="{{route('best.seller')}}" class="btn inactive" id="reports">المنتجات الأكثر مبيعا</a>
            <a href="{{route('best.profits')}}" class="btn" id="reports">المنتجات الأكثر ربحا</a>
        </div>
            ...
            <div class="my-container" id="general_view-content" >
                ...
                <div id="tap4-content">
                    <table class="table">
                        ...
                        <tr>
                        <tr>
                            @if(count($maxProfitToday) == true)
                                @foreach($maxProfitToday as $item)
                                    <td>{{$item['name']}}</td>
                                @endforeach
                            @else
                                <td>لا يوجد</td>
                            @endif
                            @if(count($maxProfitYesterday) == true)
                                @foreach($maxProfitYesterday as $item)
                                    <td>{{$item['name']}}</td>
                                @endforeach
                            @else
                                <td>لا يوجد</td>
                            @endif
                            @if(count($maxProfitThisMonth) == true)
                                @foreach($maxProfitThisMonth as $item)
                                    <td>{{$item['name']}}</td>
                                @endforeach
                            @else
                                <td>لا يوجد</td>
                            @endif
                            @if(count($maxProfitLastMonth) == true)
                                @foreach($maxProfitLastMonth as $item)
                                    <td>{{$item['name']}}</td>
                                @endforeach
                            @else
                                <td>لا يوجد</td>
                            @endif
                            @if(count($maxProfitAll) == true)
                                @foreach($maxProfitAll as $item)
                                    <td>{{$item['name']}}</td>
                                @endforeach
                            @else
                                <td>لا يوجد</td>
